...
class IB:
    ...
    **user code may not block for too long**.
    ...
    def bracketStopOrder(
            self, action: str, quantity: float,
            stopPrice: float, takeProfitPrice: float,
            stopLossPrice: float, **kwargs) -> BracketOrder:
        """
        Create a stop order that is bracketed by a take-profit order and
        a stop-loss order. Submit the bracket like:
        .. code-block:: python
            for o in bracket:
                ib.placeOrder(contract, o)
        ...
        Args:
            action: 'BUY' or 'SELL'.
            quantity: Size of order.
            stopPrice: Stop price of entry order.
            takeProfitPrice: Limit price of profit order.
            stopLossPrice: Stop price of loss order.
        """
        assert action in ('BUY', 'SELL')
        ...
        stopLoss = StopOrder(
            ...
            parentId=parent.orderId,
            **kwargs)
        return BracketStopOrder(parent, takeProfit, stopLoss)
